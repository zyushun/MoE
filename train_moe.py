...
from algorithm import muon_ours, multi_lr_adam, shampoo_ours
...
def _load_data_shard(path, num_tokens):
    # Convert string path to Path object since we're getting a string from glob.glob()
    path = Path(path)
...
class DistributedDataLoader:
    ...
    def advance(self): # advance to next data shard
        ...
        #print('self.files[self.current_shard]', self.files[self.current_shard])
...
class Hyperparameters:
    # data hyperparams
    input_bin : str = '/mntcephfs/data/ruoyusun/common_dirs/fineweb/fineweb10B/fineweb_train_*.bin' # input .bin to train on
    input_val_bin : str = '/mntcephfs/data/ruoyusun/common_dirs/fineweb/fineweb10B/fineweb_val_*.bin' # input .bin to eval validation loss on
...
if args.algorithm == 'muon':
    ...
    print0('testing adamw')
    optimizer = torch.optim.AdamW(model.parameters(), lr=0.0018, betas=(0.9, 0.95), fused=True)
    optimizers = [optimizer]
elif args.algorithm == 'adam_mini':
    print0('testing adam_mini')
    optimizer = Adam_mini(
                named_parameters = model.named_parameters(),
                lr = 0.0018,
                betas = (0.9, 0.95),
                weight_decay = args.weight_decay, # weight_decay = 0
                dim = 768,
                n_heads = 6,
                )
    optimizer.wqk_names.add('c_q')
    optimizer.wqk_names.add('c_k')
    #optimizer.wv_names.add('c_v')
    optimizer.wv_names = {}
    optimizer.attn_proj_names.add('c_proj')
    optimizers = [optimizer]
elif args.algorithm == 'multi_lr_adam':
    print0('testing multi_lr_adam')
    optimizer = multi_lr_adam(
                named_parameters = model.named_parameters(),
                lr = 0.0018,
                lr_embd = 0.6,
                lr_output = 0.008,
    ...
    optimizer.wqk_names = {'c_q', 'c_k'}
    optimizer.wv_names = {'c_v'}
    optimizers = [optimizer]
elif args.algorithm == 'muon_ours':
    print0('testing muon_ours')
    optimizer3 = muon_ours(
                named_parameters = model.named_parameters(),
                lr = 0.05,#0.0018,
                lr_embd = 0.6,
                lr_output = 0.008,
                lr_scaler = 0.04,
                betas = (0.95, 0.95), # (0.9,0.95)
                weight_decay = args.weight_decay, # weight_decay = 0
                dim = 768,
                n_heads = 6,
                )
    optimizer3.adam_block_names = {}
    optimizer3.embd_names = {"embed"}
    optimizer3.output_names = {"lm_head"}
    optimizer3.wqk_names = {'c_q', 'c_k'}
    optimizer3.wv_names = {'c_v'}
    optimizer3.attn_proj_names.add('c_proj')
    optimizers = [optimizer3]
elif args.algorithm == 'shampoo_ours':
    print0('testing shampoo_ours')
    optimizer3 = shampoo_ours(
                named_parameters = model.named_parameters(),
                lr = 0.05,#0.0018,
                lr_embd = 0.6, #0.6,
                lr_output = 0.05, #0.008,
                lr_scaler = 0.04,
                betas = (0.95, 0.95), # (0.9,0.95)
                weight_decay = args.weight_decay, # weight_decay = 0
                dim = 768,
    ...
    optimizer3.default_block_names = {'c_v', 'c_proj', 'mlp'}
    optimizer3.wqk_names = {'c_q', 'c_k'}
    optimizer3.wv_names = {}
    optimizer3.mlp_names = {}
    #optimizer3.attn_proj_names = {}
    # optimizer3.wv_names = {'c_v'}
    # optimizer3.attn_proj_names.add('c_proj')
    optimizers = [optimizer3]
...
for step in range(args.num_iterations + 1):
    ...
    if (last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0)):
        ...
        print0(f'algo: {args.algorithm} step:{step}/{args.num_iterations} val_loss:{val_loss:.4f} train_time:{training_time_ms:.0f}ms step_avg:{training_time_ms/(timed_steps-1):.2f}ms')
    ...
    print0(f"step:{step+1}/{args.num_iterations} loss:{loss:.4f} train_time:{approx_time:.0f}ms step_avg:{approx_time/timed_steps:.2f}ms")
    if master_process and args.use_wandb:
        wandb.log({"train_loss": loss})
        wandb.log({"train_time": approx_time})
        wandb.log({"train_step_avg": approx_time/timed_steps})

print0(f"peak memory consumption: {torch.cuda.max_memory_allocated() // 1024 // 1024} MiB")
if master_process and args.use_wandb:
    wandb.log({"peak_memory_consumption": torch.cuda.max_memory_allocated() // 1024 // 1024})
# -------------------------------------------------------------------------
# clean up nice
